...
import {getAnswers, getQuestionNumber, sendAnswer, setQuestionNumber} from "../../api/questions";
...
const Web3Page: FC<IWeb3PagerProps> = ({activeTab, setActiveTab}) => {
  ...
  const nextButtonHandler = async (eventData: any) => {
    ...
    if (isValid) {
      setAnswers(prev => ({...prev, [`questionNumber${currentQuestion}`]: eventData[`questionNumber${currentQuestion}`]}))
      const responseAnswers = await sendAnswer({...answers, [`questionNumber${currentQuestion}`]: eventData[`questionNumber${currentQuestion}`]})
  ...
  return (
    <main className={styles.mainPage}>
      <div className={styles.content}>
          ...
          {navigationTabs.map((item: INavigationTabs) =>
            <li
              ...
              className={activeTab === item.id
                ? [styles.navigationItem, styles.navigationItemActive].join(' ')
        ...
        {activeTab === 'entrepreneurToolkit' && <form className={styles.guideBox} onSubmit={handleSubmit(nextButtonHandler)}>
          <h2 className={styles.guideTitle}>Setup Guide</h2>
          <p className={styles.guideInfoText}>Unlock your highest potential with our personalized guide!</p>
          <p className={styles.guideInfoText}>{currentQuestion - 1}/7 Questions answered</p>
          <div className={styles.progressWrapper}>
            <QuestionsProgress currentStep={currentQuestion - 1} totalAmount={7} />
          </div>
          <div className={styles.questionBox}>
            ...
            <p className={styles.question}>{questionsStep[currentQuestion].question}</p>
          </div>
          <div className={styles.radioBlock}>
            ...
            <p className={styles.errorMessage}>{errors[`questionNumber${currentQuestion}`]?.message}</p>}
          </div>
          {currentQuestion <= 7
            ? <div className={styles.buttonBox}>
              <Button buttonText={'Cancel'} onClick={cancelButtonHandler} variant={'cancel'}/>
