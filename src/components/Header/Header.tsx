...
const Header: FC<IHeaderProps> = ({ activeTab, setMenuVision, menuVision }) => {
  ...
  const pageIcon =
    mainTabs.find((item: IMainItem) => item.id === tab)?.icon || null;
  const pageTitle =
    mainTabs.find((item: IMainItem) => item.id === tab)?.text || '';
  const rootRoute =
    mainTabs.find((item: IMainItem) => item.id === tab)?.shortText || '';
  const addRoute =
    navigationTabs.find((item: INavItem) => item.id === activeTab)?.text || '';
      ...
      <header className={styles.header}>
        {pageTitle ? (
          <div className={styles.infoBox}>
            <div className={styles.iconWrapper}>{pageIcon}</div>
            <div className={styles.info}>
              <h1 className={styles.pageName}>{pageTitle}</h1>
              <p className={styles.pageNav}>
                {`${rootRoute} ${addRoute && '/'} ${addRoute}`}
              </p>
            </div>
